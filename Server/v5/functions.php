...
function showAll($active,$domain,$export)
{
	...
	if (_ENCRYPT_=="TRUE")
	{
		$szRequest="select id,categId,domainId,".$param_szName.",".$param_title.",".$param_url.",typeapp,bKBSim,id1Name,pwdName,".
	...
	else
	{
		$szRequest="select id,categId,domainId,".$param_szName.",".$param_title.",".$param_url.",typeapp,bKBSim,id1Name,pwdName,".
				   "validateName,szKBSim,id2Name,id2Type,id3Name,id3Type,id4Name,id4Type,".$param_szFullPathName.
				   ",lastModified,pwdGroup,autoLock from "._TABLE_PREFIX_."config where active=".$active." ".
				   $szWhere." order by domainId,categId,id";
	}
	...
	if ($export==0)
	{
		...
		if ($active==1)
			...
			echo "<font face=verdana size=2><b><a href=./admin.php?action=menu"._MENUSUFFIX_.">Menu principal</a> > <a href=./admin.php?action=menu"._MENUSUFFIX_."&domain=".$domain.">Gestion du domaine ".getDomainLabel($cnx,$domain)."</a> > Liste des configurations archiv&eacute;es (<a href=./export.php?data=archived&domain=".$domain.">Exporter</a>)</b><br/><br/>";
		echo $title;
		if ($active==1)
	...
	else // $export=1
	{
		...
		$header="Id."._SEPARATOR_."Categ."._SEPARATOR_."Domaine"._SEPARATOR_."Nom"._SEPARATOR_."Titre"._SEPARATOR_."URL"._SEPARATOR_."Type"._SEPARATOR_."KBSim?"._SEPARATOR_.
	...
	for ($i=0;$i<mysql_num_rows($req);$i++)
	{
		...
		if ($export==0)
		{
			...
			echo "<td>".utf8_encode($ligne[4])."</td>";                                                           // title
			if ($ligne[5]!="") echo "<td>".utf8_encode($ligne[5])."</td>"; else echo "<td align=center>-</td>";   // URL
			echo "<td>".utf8_encode($ligne[6])."</td>";                                                           // typeapp
			echo "<td>".utf8_encode($ligne[7])."</td>";                                                           // bKBSim
			if ($ligne[8]!="") echo "<td>".utf8_encode($ligne[8])."</td>"; else echo "<td align=center>-</td>";   // id1Name
			if ($ligne[9]!="") echo "<td>".utf8_encode($ligne[9])."</td>"; else echo "<td align=center>-</td>";   // pwdName
			if ($ligne[10]!="") echo "<td>".utf8_encode($ligne[10])."</td>"; else echo "<td align=center>-</td>";   // validateName
			if ($ligne[11]!="") echo "<td>".utf8_encode($ligne[11])."</td>"; else echo "<td align=center>-</td>";   // szKBSim
			if ($ligne[12]!="") echo "<td>".utf8_encode($ligne[12])."(".utf8_encode($ligne[13]).")</td>"; else echo "<td align=center>-</td>";    // id2Name+id2Type
			if ($ligne[14]!="") echo "<td>".utf8_encode($ligne[14])."(".utf8_encode($ligne[15]).")</td>"; else echo "<td align=center>-</td>";   // id3Name+id3Type
			if ($ligne[16]!="") echo "<td>".utf8_encode($ligne[16])."(".utf8_encode($ligne[17]).")</td>"; else echo "<td align=center>-</td>"; // id4Name+id4Type
			if ($ligne[18]!="") echo "<td>".utf8_encode($ligne[16])."</td>"; else echo "<td align=center>-</td>";   // szFullPathName
			if ($ligne[19]!="") echo "<td>".utf8_encode($ligne[19])."</td>"; else echo "<td align=center>-</td>";   // lastModified
