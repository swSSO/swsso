...
{
	$var_domain=addslashes($_GET['domain']);
...
else if ($_GET['action']=="showlogs"._READSUFFIX_)
{
	$var_domain=addslashes($_GET['domain']);
	$var_result="";
	if (isset($_GET["result"])) $var_result=addslashes($_GET['result']);
...
else if ($_GET['action']=="archiveconfig"._WRITESUFFIX_)
{
	...
	if (isset($_GET["id"])) 	$var_id=addslashes($_GET['id']);
	if (isset($_GET["active"]))	$var_active=addslashes($_GET['active']);
	$var_domain=""; if (isset($_GET["domain"]))	$var_domain=addslashes($_GET['domain']);
