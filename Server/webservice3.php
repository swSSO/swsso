...
//------------------------------------------------------------------------------
// WEBSERVICE3.PHP : Utilisé à partir de la version swSSO 0.89 BETA 2
//                   (les versions précédentes utilisent webservice2.php)
//------------------------------------------------------------------------------
//
...
else if ($_GET['action']=="getconfig")
{
      ...
      if ($var_url=="")
      {
         ...
         $szRequest="select typeapp,title,url,id1Name,pwdName,id2Name,id2Type,id3Name,id3Type,id4Name,id4Type,id5Name,id5Type,validateName,bKBSim,szKBSim,szName ".
...
mysql_query($szLogRequest,$cnx);
      ...
      else
      {
         ...
         while($ligne=mysql_fetch_row($req))
         {
            ...
            echo "    <szKBSim><![CDATA[".stripslashes($ligne[15])."]]></szKBSim>";
            echo "    <szName><![CDATA[".stripslashes($ligne[16])."]]></szName>";
            echo "</app>";
            $i++;
         }
         echo "</apps>";
...
else if ($_GET['action'] =="putconfig")
{
	$cnx=dbConnect();
    header("Content-type: text/xml; charset=UTF-8");
    //http://www.swsso.fr/server/webservice.php?action=putconfig&title=titrefenetre&url=url&typeapp=WIN&id1Name=champidentifiant&id1Type=EDIT&pwdName=champmotdepasse&validateName=boutonvalidation&bKBSim=1&szKBSIM=[ID][TAB]...&szName=
    ...
    $var_bKBSim		=utf8_decode(addslashes($_GET['bKBSim'])); 
    $var_szKBSim		=utf8_decode(addslashes($_GET['szKBSim']));
    $var_szName		=utf8_decode(addslashes($_GET['szName']));
    ...
    if ($result)
    {
		$szRequest="insert into config (active,typeapp,title,url,id1Name,id1Type,pwdName,validateName,".
		           "id2Name,id2Type,id3Name,id3Type,id4Name,id4Type,id5Name,id5Type,bKBSim,szKBSim,szName) ".
		           "values (1,'".$var_typeapp."','".$var_title."','".$var_url."','".$var_id1Name."','EDIT','".
		           $var_pwdName."','".$var_validateName."','".$var_id2Name."','".$var_id2Type."','".
		           $var_id3Name."','".$var_id3Type."','".$var_id4Name."','".$var_id4Type."','".
		           $var_id5Name."','".$var_id5Type."',".$var_bKBSim.",'".$var_szKBSim."','".$var_szName."')";
...
else if ($_GET['action']=="showall08235ekop")
{
	$cnx=dbConnect();
      ...
      $szRequest="select szName,title,url,typeapp,bKBSim,id1Name,pwdName,validateName,szKBSim,id2Name,id2Type,id3Name,id3Type,id4Name,id4Type,id5Name,id5Type from config where active=1 order by id desc";
      if ($_GET['debug08235ekop']!="") echo $szRequest;
      $req=mysql_query($szRequest,$cnx);
      ...
      for ($i=0;$i<mysql_num_rows($req);$i++)
      {
         ...
         echo "<tr>";
         if ($ligne[0]!="") echo "<td>".utf8_encode($ligne[0])."</td>"; else echo "<td align=center>-</td>";   // szName
         echo "<td>".utf8_encode($ligne[1])."</td>";                                                           // title
         if ($ligne[2]!="") echo "<td>".utf8_encode($ligne[2])."</td>"; else echo "<td align=center>-</td>";   // URL
         echo "<td>".utf8_encode($ligne[3])."</td>";                                                           // typeapp
         echo "<td>".utf8_encode($ligne[4])."</td>";                                                           // bKBSim
         if ($ligne[5]!="") echo "<td>".utf8_encode($ligne[5])."</td>"; else echo "<td align=center>-</td>";   // id1Name
         if ($ligne[6]!="") echo "<td>".utf8_encode($ligne[6])."</td>"; else echo "<td align=center>-</td>";   // pwdName
         if ($ligne[7]!="") echo "<td>".utf8_encode($ligne[7])."</td>"; else echo "<td align=center>-</td>";   // validateName
         if ($ligne[8]!="") echo "<td>".utf8_encode($ligne[8])."</td>"; else echo "<td align=center>-</td>";   // szKBSim
         if ($ligne[9]!="") echo "<td>".utf8_encode($ligne[9])."(".utf8_encode($ligne[10]).")</td>"; else echo "<td align=center>-</td>";    // id2Name+id2Type
         if ($ligne[11]!="") echo "<td>".utf8_encode($ligne[11])."(".utf8_encode($ligne[12]).")</td>"; else echo "<td align=center>-</td>";   // id3Name+id3Type
         if ($ligne[13]!="") echo "<td>".utf8_encode($ligne[13])."(".utf8_encode($ligne[14]).")</td>"; else echo "<td align=center>-</td>"; // id4Name+id4Type
         echo "</tr>";
      }
      echo "</table>";
      echo "<br/><a href=./webservice3.php?action=showold08235ekop style=\"font-family:Verdana; font-size:11px;\">Voir les configurations archiv&eacute;es</a>";
      echo "</html>";
	mysql_close($cnx);
}
// ------------------------------------------------------------
...
else if ($_GET['action']=="showold08235ekop")
{
	$cnx=dbConnect();
      header("Content-type: text/html; charset=UTF-8");
      $szRequest="select szName,title,url,typeapp,bKBSim,id1Name,pwdName,validateName,szKBSim,id2Name,id2Type,id3Name,id3Type,id4Name,id4Type,id5Name,id5Type from config where active=0 order by id desc";      
      if ($_GET['debug08235ekop']!="") echo $szRequest;
      $req=mysql_query($szRequest,$cnx);
      if (!$req) {echo "<html><p>Erreur : ". mysql_errno($cnx) . mysql_error($cnx)."</p></html>";return;}
...
else if ($_GET['action']=="showstats")
{
      ...
      if (!$req) {echo "<html><p>Erreur : ". mysql_errno($cnx) . mysql_error($cnx)."</p></html>";return;}
      $ligne=mysql_fetch_row($req);
      echo "<font face=verdana size=2>Statistiques - Nombre d'appels getversion depuis le 5/12/2009 : ".$ligne[0]."</font><br/>";
