						  ...
						  char *szDate, BOOL bNew,BOOL bMod,BOOL bOld,int iType)
{
	...
	pszResult=HTTPRequest(gszServerAddress,giServerPort,pszRequest,8,NULL,NULL);
...
int PutConfigOnServer(int iAction,int *piNewCategoryId,char *pszDomainIds)
{
	...
	*piNewCategoryId=-1;
	...
	pszResult=HTTPRequest(gszServerAddress,giServerPort,szRequest,5,NULL,NULL); // timeout : 5 secondes
...
int InternetCheckProxyParams(HWND w)
{
	...
	TRACE((TRACE_INFO,_F_,"Requete HTTP : %s",szRequest));
	pszResult=HTTPRequest(gszServerAddress,giServerPort,szRequest,5,&ProxyParams,NULL);
	if (pszResult==NULL) { TRACE((TRACE_ERROR,_F_,"HTTPRequest(%s)=NULL",szRequest)); goto end; }
...
void InternetCheckVersion()
{
	...
	sprintf_s(szRequest,sizeof(szRequest),"%s?action=getversion",gszWebServiceAddress);
	TRACE((TRACE_INFO,_F_,"Requete HTTP : %s",szRequest));
	pszResult=HTTPRequest(gszServerAddress,giServerPort,szRequest,3,NULL,NULL);
	if (pszResult==NULL) { TRACE((TRACE_ERROR,_F_,"HTTPRequest(%s)=NULL",szRequest)); goto end; }
