...
int GetConfigHeader()
{
	...
	if (giRecoveryInfosKeyId==giRecoveryKeyId_ChangeValue) 
	{
		...
		TRACE((TRACE_INFO,_F_,"KeyId du RecoveryInfos=%04d KeyId de ChangeIniValues=%04d KeyId à appliquer=%04d",giRecoveryInfosKeyId,giRecoveryKeyId_ChangeValue,giRecoveryKeyId));
						  ...
						  char *szDate, BOOL bNew,BOOL bMod,BOOL bOld,int iType,int iAutoPublish)
{
	...
	pszResult=HTTPRequest(gszServerAddress,giServerPort,gbServerHTTPS,gszWebServiceAddress,
						  ...
						  pszParams,L"GET",NULL,0,NULL,WINHTTP_AUTOLOGON_SECURITY_LEVEL_HIGH,8,NULL,&dwStatusCode);
...
int PutConfigOnServer(int iAction,int *piNewCategoryId,char *pszDomainIds)
{
	...
	char szType[3+1];
	int rc=-1;
	char szParams[2048+1]; // augmenté en 1.02 pour ISSUE#160 (taille champ URL 128 -> 256)
	char *pszResult=NULL;
	char szId2Type[5+1]="";
	char szId3Type[5+1]="";
	char szId4Type[5+1]="";
	...
	*piNewCategoryId=-1;
	...
	pszResult=HTTPRequest(gszServerAddress,giServerPort,gbServerHTTPS,gszWebServiceAddress,
						  gszServerAddress2,giServerPort2,gbServerHTTPS2,gszWebServiceAddress2,
						  szParams,L"GET",NULL,0,NULL,WINHTTP_AUTOLOGON_SECURITY_LEVEL_HIGH,5,NULL,&dwStatusCode); // timeout : 5 secondes
