...
// swKeystoreSave()
//-----------------------------------------------------------------------------
// Sauvegarde du keystore contenant des clés privées 
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Obtient la clé au format SZ pour stockage dans le keystore
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Importe une nouvelle clé privée dans le keystore
//-----------------------------------------------------------------------------
// [in] szPrivateKeyFile : fichier contenant la clé à importer
// [in] szPrivateKeyPassword : mot de passe protégeant la clé
...
int swKeystoreImportPrivateKey(char *szPrivateKeyFile, char *szPrivateKeyPassword, char *szKeystorePassword)
{
	...
	// lit la ligne (unique, au format id:sel:clé)
	...
	// vérifie que cette clé n'est pas déjà dans le keystore
	for (i=0;i<giNbPrivateKeys;i++)
	{
		if (gtabPrivateKey[i].iKeyId==gtabPrivateKey[giNbPrivateKeys].iKeyId) 
		{ 
			TRACE((TRACE_ERROR,_F_,"La clé %04d est déjà dans le keystore",gtabPrivateKey[i].iKeyId));
			rc=SWCRYPT_KEYEXISTS; 
			goto end; 
		}
	}
	// lit le sel associé à la clé
	memcpy(gtabPrivateKey[giNbPrivateKeys].szSaltData,buf4096+5,PBKDF2_SALT_LEN*2);
	...
	// importe la clé dans le CSP en la déchiffrant avec le mot de passe "szPrivateKeyPassword"
...
void swCryptErrorMsg(HWND w,int ret)
{
	...
	switch(ret)
	{
		...
		case SWCRYPT_FILEWRITE:
			...
			break;
		case SWCRYPT_ERROR: // pas de break, c'est fait exprès
		default:
